...
public class GBMModel extends SharedTreeModel<GBMModel, GBMModel.GBMParameters, GBMModel.GBMOutput> {
  ...
  public static class GBMOutput extends SharedTreeModel.SharedTreeOutput {
    ...
    public GBMOutput(GBM b) {
      super(b);
      _binomial = (b._parms._distribution == DistributionFamily.quasibinomial || b._parms._distribution == DistributionFamily.bernoulli);
      _nclasses = b.nclasses();
      if(_binomial && _domains[_domains.length-1] != null)
        assert _domains[_domains.length - 1].length == 2:"Unexpected domains " + Arrays.toString(_domains);
    }
    @Override
